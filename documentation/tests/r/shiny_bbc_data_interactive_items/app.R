  ...
  sidebarLayout(
    sidebarPanel(
      ...
      radioButtons("condition", label = h3("Conditions"),
                   ...
                   selected = 1),
      checkboxGroupInput("items", label = h3("Actions"), 
                         choices = list("Recipe 1" = "tab-r1", "Recipe 2" = "tab-r2", "Recipe 3" = "tab-r3", "Recipe 4" = "tab-r4"),
                         selected = "play"),
...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    plotBBCdata_items(bbc_data, input$condition, c(input$items), input$time[1],input$time[2])
